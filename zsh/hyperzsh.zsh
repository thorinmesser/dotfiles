...
# The prompt
PROMPT='
$(_user_host)$(_python_venv)%{$fg[blue]%}%~ $(git_prompt_info)%{$reset_color%}$(git_prompt_status)${_return_status}'
PROMPT+='
%(?.%F{white}.%F{red})❯%f '

# Prompt with SHA
# PROMPT='$(_user_host)$(_python_venv)%{$fg[blue]%}%c $(git_prompt_info)%{$reset_color%}$(git_prompt_short_sha)%{$fg[magenta]%}$(_git_time_since_commit)$(git_prompt_status)${_return_status}➜ '
